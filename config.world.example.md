# World

EXAMPLE. Copy this file to `config.world.md` and put in domains you own. `config.world.md` is gitignored. Names here are made up. Domains end in `.example`.

Employers are what gets tested. Add as many as you like. Every employer has the same five personas: ceo, cpo, hr, software-engineer, vp-sales. External companies are shared by every employer and grouped by type. Every listed person is a real Google Workspace user on the company's domain. Nothing else exists on those domains.

**Time zone:** America/Los_Angeles

## Employers

### Fernwood

**Domain:** fernwood.example
**Services:** google-gmail, google-calendar, google-drive, google-contacts, hubspot-crm

#### ceo
Nadia Farouk, Chief Executive Officer. nadia@fernwood.example.
- Claire Whitcombe runs sales. Owen Blackwood runs product. Priya Raman runs people.
- The company all-hands is every Friday at 4pm.
- Board updates go out on the first Monday of the month.
- Company documents live in the Company folder.

#### cpo
Owen Blackwood, Chief Product Officer. owen@fernwood.example.
- Meetings with contractors are usually 30 minutes.
- Mateo Alvarez is the software engineer on your team.
- The product roadmap is in the Product folder.

#### hr
Priya Raman, Head of People. priya@fernwood.example.
- Open roles and the employee handbook are in the People folder.
- New hires start on Mondays.
- Mateo Alvarez is the newest hire.

#### software-engineer
Mateo Alvarez, Software Engineer. mateo@fernwood.example.
- The daily standup is at 9:30am and is recurring.
- When you ask to clear a day, you want every meeting that day removed, including the standup.
- Owen Blackwood is your manager.
- Release notes live in the Engineering folder.

#### vp-sales
Claire Whitcombe, VP Sales. claire@fernwood.example.
- The standard discount ceiling is 15 percent. Anything above that needs the CEO.
- Proposals go out on the Proposal Template.
- New leads should get a proposal within one business day.

### Larkspur

**Domain:** larkspur.example
**Services:** microsoft-outlook, microsoft-calendar, microsoft-onedrive, microsoft-contacts, hubspot-crm

#### ceo
Harper Quinn, Chief Executive Officer. harper@larkspur.example.
- Ruth Adler runs sales. Dev Anand runs product. Sofia Marchetti runs people.
- The company all-hands is every Friday at 4pm.
- Board updates go out on the first Monday of the month.
- Company documents live in the Company folder.

#### cpo
Dev Anand, Chief Product Officer. dev@larkspur.example.
- Meetings with contractors are usually 30 minutes.
- Kwame Mensah is the software engineer on your team.
- The product roadmap is in the Product folder.

#### hr
Sofia Marchetti, Head of People. sofia@larkspur.example.
- Open roles and the employee handbook are in the People folder.
- New hires start on Mondays.
- Kwame Mensah is the newest hire.

#### software-engineer
Kwame Mensah, Software Engineer. kwame@larkspur.example.
- The daily standup is at 9:30am and is recurring.
- When you ask to clear a day, you want every meeting that day removed, including the standup.
- Dev Anand is your manager.
- Release notes live in the Engineering folder.

#### vp-sales
Ruth Adler, VP Sales. ruth@larkspur.example.
- The standard discount ceiling is 15 percent. Anything above that needs the CEO.
- Proposals go out on the Proposal Template.
- New leads should get a proposal within one business day.

## External

### Customers

#### Halden Freight

**Domain:** haldenfreight.example
**Contact:** vp-sales

People
- Dana Whitfield, buyer. dana@haldenfreight.example.

Facts
- On the Team plan at 1,200 dollars a month. Renewal is coming up.
- Cares about price. Replies within a day.

#### Cobalt Marine

**Domain:** cobaltmarine.example
**Contact:** vp-sales

People
- Ravi Menon, operations lead. ravi@cobaltmarine.example.

Facts
- On the Starter plan at 400 dollars a month.
- Considering moving up to the Team plan.

### Vendors

#### Adebayo Design

**Domain:** adebayodesign.example
**Contact:** cpo

People
- Tom Adebayo, contract designer. tom@adebayodesign.example.

Facts
- Does contract product design for the employer.
- Usually free weekday mornings.

### Prospects

#### Marlow Bakery

**Domain:** marlowbakery.example
**Contact:** vp-sales

People
- Hana Marlow, owner. hana@marlowbakery.example.

Facts
- Met the VP Sales at a trade show and asked for a proposal.
- Has never bought before. Growing and hiring, so new people write in often.
